...
#define TSS_STREAM_DURATION_INFINITE 0xffffffff
	...
	***********************************************/
	...
	typedef enum TSS_RESPONSE_HEADER
	{
	...
	***********************************************/
	typedef enum TSS_TYPE
	{
		...
		TSS_NANO = 1024,			/**< 3-Space Nano */
		TSS_MINIBT = 2048,          /**< 3-Space MiniBT */
		TSS_FIND_ALL_KNOWN = 4094,	/**< Find all serial ports that have 3-Space devices connected */
        TSS_FIND_ALL = 4095			/**< Find all serial ports including "unknown" serial ports that may have 3-Space devices connected */
	} TSS_TYPE;
	...
	***********************************************/
	typedef enum TSS_CONNECTION_TYPE
	{
	...
	***********************************************/
	typedef struct TSS_ComPort
	{
		...
		U16 device_type;      /**< The type of TSS device connected through the serial port. */
		U8 connection_type;   /**< The type of connection to the TSS device. */
	} TSS_ComPort;
...
#define TSS_EXPORT
	...
	* \return TSS_NO_ERROR(0) if successful, non zero if an error occurred.
	***********************************************/
	TSS_EXPORT TSS_ERROR tss_sensor_getHardwareVersionString(tss_device_id sensor_id, char* hw_version_string, U32* timestamp);
	/********************************************//**
	* \ingroup tss_sensor_methods
	...
	* \return TSS_NO_ERROR(0) if successful, non zero if an error occurred.
	***********************************************/
	TSS_EXPORT TSS_ERROR tss_sensor_getFirmwareVersionString(tss_device_id sensor_id, char* fw_version_string, U32* timestamp);
    ...
    ***********************************************/    
	...
	/********************************************//**
	* \ingroup tss_sensor_methods
	* \brief Return the oldest received packet in raw buffer form.
	* \param[in] sensor_id The tss_device_id that represents the 3-Space Sensor.
	* \param[out] raw_stream_packet The last packet in the buffer.
	* \return TSS_NO_ERROR(0) if successful, non zero if an error occurred.
	***********************************************/
	TSS_EXPORT TSS_ERROR tss_sensor_getFirstStreamingPacketRaw(tss_device_id sensor_id, U8* raw_stream_packet);
    /********************************************//**
    * \ingroup tss_sensor_methods
    ...
    ***********************************************/
	...
	/********************************************//**
	* \ingroup tss_sensor_methods
	* \brief Return the newest received packet in raw buffer form.
	* \param[in] sensor_id The tss_device_id that represents the 3-Space Sensor.
	* \param[out] raw_stream_packet The last packet in the buffer.
	* \return TSS_NO_ERROR(0) if successful, non zero if an error occurred.
	***********************************************/
	TSS_EXPORT TSS_ERROR tss_sensor_getLastStreamingPacketRaw(tss_device_id sensor_id, U8* raw_stream_packet);
