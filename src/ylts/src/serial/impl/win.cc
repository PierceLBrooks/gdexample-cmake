    ...
    bytesize_(bytesize), stopbits_(stopbits), flowcontrol_(flowcontrol)
{
	...
	memset(&read_asynchronous, 0, sizeof(OVERLAPPED));
	memset(&write_asynchronous, 0, sizeof(OVERLAPPED));

	read_asynchronous.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
    write_asynchronous.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);

	/*
	SetCommMask(read_asynchronous.hEvent, EV_RXCHAR);
	SetCommMask(write_asynchronous.hEvent, EV_TXEMPTY);

	DWORD dwCommEvent;
	DWORD dwCommEventBK;
	GetCommMask(fd_, &dwCommEventBK);
	SetCommMask(fd_, EV_RXCHAR);
	WaitCommEvent(fd_, &dwCommEvent, &read_asynchronous);
	SetCommMask(fd_, EV_TXEMPTY);
	WaitCommEvent(fd_, &dwCommEvent, &write_asynchronous);
	SetCommMask(fd_, dwCommEventBK);
	*/
...
void serial::WaitForMultiplePorts(std::vector<std::shared_ptr<Serial>>& ports)
{
    ...
    {
		DWORD dwCommEvent;
		OVERLAPPED* read = &(*it)->pimpl_->read_asynchronous;
		HANDLE fd_ = (*it)->pimpl_->fd_;
		HANDLE comPortHandle = read->hEvent;
		
		SetCommMask(fd_, EV_RXCHAR);
		WaitCommEvent(fd_, &dwCommEvent, read);
        
		port_handles.push_back(comPortHandle);		
    }
	
    DWORD error = WaitForMultipleObjects(port_handles.size(), port_handles.data(), false, 1000);
}
#endif // #ifdef _WIN32